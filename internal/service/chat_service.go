...
type ChatServiceInterfact interface {
	...
	GetHistoryChat(roomIDStr string) ([]models.ChatMessage, error)
	CreateRoom(name string) (primitive.ObjectID, error)
}
...
func (s *ChatService) GetHistoryChat(roomIDStr string) ([]models.ChatMessage, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	room, err := s.ChatRepository.FindRoomChatByID(ctx, roomID)
	if err != nil {
		return nil, errors.New("chat room not found")
	}

	return room.ChatMessages, nil
}
